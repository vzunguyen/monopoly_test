...
describe 'PredefinedDice' do
  ...
  describe '#roll' do
    it 'raises error if roll number is more than 6' do
      ...
      expect { dice.roll }.to raise_error(RuntimeError, /ERROR: Roll need to be an number and in range 1-6/)
    end
  ...
  describe '#is_end?' do
    ...
    it 'overrolls the dice' do
      new_dice = PredefinedDice.new(rolls_data: [1])
      expect(new_dice.roll).to eq(1)
      expect { new_dice.roll }.to raise_error(RuntimeError, /ERROR: Roll need to be an number and in range 1-6/)
    end
  end
end
